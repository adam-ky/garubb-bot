...
RegisterSuccessHandler = async (ctx) => {
    ...
    if (angel.isRegistered()) {
        ...
        await ctx.model.angelBot.telegram.sendMessage(person.telegramId, messages.RegisteredNotifier(ctx.chatTarget))
        await ctx.model.angelBot.telegram.sendMessage(angel.telegramId, messages.RegisteredNotifier(ctx.chatTarget))   
    } else {
        await ctx.model.angelBot.telegram.sendMessage(person.telegramId, messages.UnregisteredNotifier(ctx.chatTarget))
    }
...
HelpHandler = async (ctx) => {
    await ctx.replyWithMarkdown(messages.HelpMessage(ctx.chatTarget))
}
